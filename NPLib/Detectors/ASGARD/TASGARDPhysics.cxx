...
/////////////////////////////////////////////////
TVector3 TASGARDPhysics::GetCloverPosition(int& CloverNbr){
...
void TASGARDPhysics::InitializeRootInputRaw() {
  ...
  if(inputChain->FindBranch( "fASG_*" ))
    inputChain->SetBranchStatus( "fASG_*" , true );
...
void TASGARDPhysics::AddParameterToCalibrationManager(){
  ...
  for(int i = 0 ; i < 16; ++i){
    for(int cry = 0 ; cry < 4 ; cry++){
      ...
      for( int j = 0 ; j < 10 ; ++j){
        Cal->AddParameter("ASGARD", "D"+ NPL::itoa(i+1)+"_CRY"+NPL::itoa(cry+1)+"_SEG"+ NPL::itoa(j)+"_E","ASGARD_D"+ NPL::itoa(i+1)+"_CRY"+NPL::itoa(cry+1)+"_SEG"+NPL::itoa(j)+"_E");
      }
    }
  } 
  return;
...
////////////////////////////////////////////////////////////////////////////////
NPL::VDetector* TASGARDPhysics::Construct(){
  return (NPL::VDetector*) new TASGARDPhysics();
}
