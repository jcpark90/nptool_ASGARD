...
/////////////////////////
void TASGARDData::SetNewGeData(UInt_t GeCloverNbr, UInt_t GeCrystalNbr, UInt_t GeSegmentNbr, Double_t GeEnergy, Double_t GeTimeLED, Double_t GeThetaSemiTrue, Double_t GePhiSemiTrue, TVector3 GePosition){
...
void TASGARDData::SortSegmentData(){ // loop through
  for(unsigned int i = 0; i < fASGARD_Energy_sub.size(); i++){ // 
    ...
    if (fASGARD_CloverNbr_sub[i]!=last_clover || fASGARD_CrystalNbr_sub[i]!=last_crystal){ // new clover/crystal, register
      SetNewGeData(fASGARD_CloverNbr_sub[i],fASGARD_CrystalNbr_sub[i], fASGARD_SegmentNbr_sub[i], fASGARD_Energy_sub[i], fASGARD_TimeLED_sub[i], fASGARD_Theta_SemiTrue_sub[i], fASGARD_Phi_SemiTrue_sub[i], fASGARD_position_sub[i]);
