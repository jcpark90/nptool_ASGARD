...
namespace PLASTIC{
  ...
  const G4double ResoTime    = 0.150*ns         ;// Resolution  //
...
void Plastic::VolumeMaker(G4ThreeVector Det_pos, int DetNumber, G4LogicalVolume* world){
  ...
  if(m_PlasticRadius[i]!=-1){
    if(m_PlasticThickness[i]>0 && m_PlasticRadius[i]>0){
      ...
      G4LogicalVolume* logicPlastic = new G4LogicalVolume(solidPlastic, PlasticMaterial, "LogicScintillator", 0, 0, 0);
  ...
  if(m_PlasticHeight[i]!=-1){
    if(m_PlasticThickness[i]>0 && m_PlasticHeight[i]>0 && m_PlasticWidth[i]>0){
      ...
      G4LogicalVolume* masterPlastic = new G4LogicalVolume(solidPlastic, Vacuum, "Scintillator", 0, 0, 0);
      masterPlastic->SetVisAttributes(G4VisAttributes::Invisible);
      //      G4LogicalVolume* logicPlastic = new G4LogicalVolume(solidPlastic, PlasticMaterial, Name+ "_Scintillator", 0, 0, 0);
      G4LogicalVolume* logicPlastic = new G4LogicalVolume(solidPlastic, PlasticMaterial, "LogicScintillator", 0, 0, 0);
...
void Plastic::ReadSensitive(const G4Event*){
  ...
  for(unsigned int i=0; i<size; i++){
    ...
    if(Energy>EnergyThreshold){
      //      G4cout<<DetectorNbr<<" "<<Energy/keV<<" "<<Time<<G4endl;
      m_Event->SetEnergyAndTime(DetectorNbr,Energy/keV,Time);
    }
  }  
...
void Plastic::InitializeScorers() { 
  ...
  G4VPrimitiveScorer* Calorimeter = new CalorimeterScorers::PS_Calorimeter("Plastic",level);
