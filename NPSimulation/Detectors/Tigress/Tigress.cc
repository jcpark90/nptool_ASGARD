...
Tigress::~Tigress(){
  // JP: we never use "vacuum" as a material
...
void Tigress::ReadSensitive(const G4Event* event){
  ...
  for (HPGE_itr = HPGEHitMap->GetMap()->begin() ; HPGE_itr != HPGEHitMap->GetMap()->end() ; HPGE_itr++){
    ...
    G4double Energy_true = Info[0]/keV;
    //    G4double Energy_reso = 0.4*keV + Energy_true*0.0015; // FWHM
    G4double Energy_reso = (1.5 + Energy_true*0.0015)/2.355; // sigma
    
    G4double Energy   = Energy_true;
    //    if (Energy_true > 30*keV && Energy_true < 110*keV)
    //      G4cout<<Energy_true<<" "<<Energy<<G4endl;
    ...
    G4int SegmentNbr = 0;
    G4double xpos = Info[2]; // mm
    G4double ypos = Info[3]; // mm
    G4double zpos = Info[4]; // mm
    //    G4ThreeVector vpos(xpos,ypos,zpos);
    //    G4cout<<CloverNbr<<" "<<CrystalNbr<<" "<<vpos<<G4endl;
    ...
    if (Energy > 30){ // JP: increased from 1*keV
      m_TigressData->SetGeCloverNbr(CloverNbr);
      m_TigressData->SetGeCrystalNbr(CrystalNbr);
      m_TigressData->SetGeSegmentNbr(SegmentNbr);
      m_TigressData->SetGeTimeLED(RandGauss::shoot(Time, 15)-0.5); // 15-ns sigma, 15-cm from 
      m_TigressData->SetGeEnergy(RandGauss::shoot(Energy, Energy_reso));
    }
  }	
