...
const double I152EU_HPGE[10] = {7.5,  4.2, 14.5, 13.6, 21, 2.234, 12.942,  1.623, 28.58, 59.4};
...
void calib_singles_coinc_magnet(){
  gStyle->SetOptStat(0);
  ...
  for(int i = 0; i < 29; i++){
    ...
    ffatima[i]->SetParameter(0, hfatima[i]->GetMaximum());     ffatima[i]->SetParLimits(0, 10, 1e5);
  ...
  legend->AddEntry(gidaten, "IDATEN (82 ch)mono", "lp");
  legend->AddEntry(gfatima, "FATIMA (36 ch) mono", "lp");
  ...
  cg->SaveAs("idaten_hpge_magnet_152Eu_efficiency_2000keV.png");
  
  cg->cd()->SetLogx(1);
  gidaten->GetYaxis()->SetNdivisions(11, 5, 0);
  gidaten->GetYaxis()->SetRangeUser(0, 0.21);
  cg->SaveAs("idaten_hpge_magnet_152Eu_efficiency_logx.png");
