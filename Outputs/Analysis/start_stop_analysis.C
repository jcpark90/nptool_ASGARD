...
const int GATE_SET = 2; // 2 settings for examining the half-life of interest in 96Pd
const double TIME_CUTOFF = 10000; // 10 us, beyond which the spectrum may be mostly 138La background

void start_stop_analysis(){
  gStyle->SetOptStat(0);

  if (GATE_SET==1){
    // set 1 for 7.5-ns 6+ state in 96Pd
    EGATE_START = 106; // 106 keV to 6+ state in 96Pd
    EWIND_START = 10; // 10 keV
    EGATE_STOP = 325; // 325 keV from 6+ state in 96Pd
    EWIND_STOP = 20; // 20 keV
  }
  else if (GATE_SET==2){
    // set 2 for 1.0-ns 4+ state in 96Pd
     EGATE_START = 325; // 106 keV to 6+ state in 96Pd
     EWIND_START = 20; // 20 keV
     EGATE_STOP = 684; // 325 keV from 6+ state in 96Pd
     EWIND_STOP = 30; // 30 keV
  }
  teu = new TChain("SimulatedTree");
  teu->Add("96Pd_passive.root");
  fReader.SetTree(teu);
  ...
  if (GATE_SET==1)
    ...
    hdt_idaten = new TH1D("hdt_idaten", "", 240, -2, 10); // start-stop time difference histogram for 1.0-ns state
  ...
  if (GATE_SET==2){
    fhalf = new TF1("fhalf", "[1]*(exp(0.5*(log(2)/[0])*((log(2)/[0])*[2]**2-2*x))*TMath::Erfc(((log(2)/[0])*[2]**2-x)/(sqrt(2)*[2])))", -1, 10);
  ...
  if (GATE_SET==1){
    latex->DrawLatex(20, 30, "^{96}Pd: 106(start)-325(stop)");
    ...
    cc->SaveAs("start_stop_analysis_96Pd_6plus.png");
  }
  else if (GATE_SET==2){
    latex->DrawLatex(4, 25, "^{96}Pd: 325(start)-684(stop)");
    latex->DrawLatex(4, 20, Form("T_{1/2}(4^{+}) = %.2f #pm %.2f ns (Sim.)", fhalf->GetParameter(0),  fhalf->GetParError(0)));
    latex->DrawLatex(4, 15, Form("T_{1/2}(4^{+}) = %.1f ns (Geant4, Lit.)", 1.0));
    cc->SaveAs("start_stop_analysis_96Pd_4plus.png");
  }
}
