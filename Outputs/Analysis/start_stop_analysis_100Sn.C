...
void GetGammaProjectionLaBr3WithPlastic(TTree* tr, TH1D* hout, double egate, double ewind){
  ...
  while(fReader.Next()){           
    ...
    if (Plastic->GetMult() > 0 && i < entries/2){
...
void GetStartStopHistWithPlastic(TTree* tr, TH1D* hout, double estop, double ewind_stop){
  ...
  while(fReader.Next() && i < entries/2){ // condition for reduced beam time          
    i++;
    if((i % 100000)==0) cout << i << endl;
...
void start_stop_analysis_100Sn(){
  // gStyle->SetOptStat(0);

  teu = new TChain("SimulatedTree");

  // 100In
 //  teu->Add("100In_decay.root");
 ...
 // TH1D* ht_idaten_453_290 = new TH1D("ht_idaten_453_290", "", 120, -2, 10);  
 //  GetStartStopHist(teu, ht_idaten_453_290, 453, 20, 290, 20);

  
  
 //  TF1* fhalf = new TF1("fhalf", "[1]*(exp(0.5*(log(2)/[0])*((log(2)/[0])*[2]**2-2*x))*TMath::Erfc(((log(2)/[0])*[2]**2-x)/(sqrt(2)*[2])))", 0.5, 7);
 //  fhalf->SetParameter(0, 0.5);
 //  fhalf->SetParameter(1, 25);
 //  fhalf->SetParameter(2, 1);
  
 //  ht_idaten_453_290->Fit(fhalf, "RN0");
  
 //  TFile* f100In = new TFile("outputs_100In_decay.root", "recreate");
 //  he_idaten->Write();
 //  he_hpge->Write();
 //  ht_idaten_453_290->Write();
 //  fhalf->Write();
  
 //  f100In->Close();
 //  TCanvas* cc = new TCanvas("cc", "cc", 1200, 800);
 //  cc->cd()->SetLogy(1);
 //  ht_idaten_453_290->Draw();
 //  ht_idaten_453_290->GetXaxis()->SetTitle("Time (ns)");
 //  ht_idaten_453_290->GetYaxis()->SetTitle("Counts / 0.1 ns");
 //  fhalf->Draw("same");

 //  TLatex* latex = new TLatex();
 //  latex->DrawLatex(4, ht_idaten_453_290->GetMaximum()*0.75, "^{100}Cd, 6^{+}");
 //  latex->DrawLatex(4, ht_idaten_453_290->GetMaximum()*0.3, "T_{1/2} (input)  = 1.1 ns");
 //  latex->DrawLatex(4, 30, Form("T_{1/2} = %.3lf #pm %.3lf ns", fhalf->GetParameter(0), fhalf->GetParError(0)));
 //  cc->SaveAs("100In_decay_reduced.png");
 //  cc->SaveAs("100In_decay_reduced.pdf");
 //  cc->SaveAs("100In_decay_reduced.eps");
  ...
  TH1D* he_hpge = new TH1D("he_hpge", "", 200, 0, 200);
  GetHPGeEnergySpectrum(teu, he_hpge);  
  TH1D* ht_idaten_1234_441 = new TH1D("ht_idaten_1234_441", "", 220, -2, 20);  
 //  GetStartStopHist(teu, ht_idaten_1234_441, 1234, 30, 441, 20);
  GetStartStopHistWithPlastic(teu, ht_idaten_1234_441, 441, 20);

  TF1* fhalf = new TF1("fhalf", "[1]*0.5**(x/[0])", 2, 20);
  fhalf->SetParameter(0, 3.3);
  fhalf->SetParameter(1, 25);
  //  fhalf->SetParameter(2, 1);
  //  ht_idaten_1234_441->Rebin(5);  
  ht_idaten_1234_441->Fit(fhalf, "RN0");
  
  TFile* f99In = new TFile("outputs_99In_decay.root", "recreate");
  he_idaten->Write();
  he_hpge->Write();
  ...
  TLatex* latex = new TLatex();
  latex->DrawLatex(6, ht_idaten_1234_441->GetMaximum()*0.8, "^{99}Cd, 7/2^{+}");
  latex->DrawLatex(6, ht_idaten_1234_441->GetMaximum()*0.5, "T_{1/2} (input)  = 3.3 ns");
  latex->DrawLatex(7, 150, Form("T_{1/2} = %.3lf #pm %.3lf ns", fhalf->GetParameter(0), fhalf->GetParError(0)));
  cc->SaveAs("99In_decay.png");
  cc->SaveAs("99In_decay.pdf");
  cc->SaveAs("99In_decay.eps");
  ...
  // GetHPGeEnergySpectrum(teu, he_hpge);  
  // TH1D* ht_idaten_198_688 = new TH1D("ht_idaten_198_688", "", 400, -2, 2);  
  // GetStartStopHist(teu, ht_idaten_198_688, 198, 20, 688, 20);
  
  // TFile* f98Cd = new TFile("outputs_98Cd.root", "recreate");
  // he_idaten->Write();
  // he_hpge->Write();
  // // he_idaten_labr3_gate->Write();
  // ht_idaten_198_688->Write();
  // f98Cd->Close();

  // TCanvas* cc = new TCanvas("cc", "cc", 1200, 800);
  // ht_idaten_198_688->Draw();
  // ht_idaten_198_688->Rebin(2);
  // ht_idaten_198_688->GetXaxis()->SetTitle("Time (ns)");
  // ht_idaten_198_688->GetYaxis()->SetTitle("Counts / 0.02 ns");
  // TF1* flifetime = new TF1("flifetime", "gaus", -2, 2);
  // ht_idaten_198_688->Fit(flifetime, "RN0");
  // flifetime->Draw("same");

  // TLatex* latex = new TLatex();
  // latex->DrawLatex(-1.8, ht_idaten_198_688->GetMaximum()*0.75, "^{98}Cd, 4^{+}");
  // latex->DrawLatex(-1.8, ht_idaten_198_688->GetMaximum()*0.6, "T_{1/2} (input)  = 0.027 ns");
  
  // latex->DrawLatex(-1.8, ht_idaten_198_688->GetMaximum()*0.5, Form("T_{1/2} = %.3lf #pm %.3lf ns", flifetime->GetParameter(1)*log(2), flifetime->GetParError(1)*log(2)));
  // cc->SaveAs("98Cd_27ps.png");
  // cc->SaveAs("98Cd_27ps.pdf");
  // cc->SaveAs("98Cd_27ps.eps");
