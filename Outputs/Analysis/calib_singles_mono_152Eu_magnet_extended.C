...
void calib_singles_mono_152Eu_magnet_extended(){
  gStyle->SetOptStat(0);

  teu = new TChain("SimulatedTree");
  teu->Add("idaten_hpge_magnet_extended_152Eu.root");
  ...
  for(int i = 0; i < 29; i++){
    cout<<i<<endl;
    tfatima->Reset();
    tfatima->Add(Form("idaten_hpge_magnet_extended_%dkeV.root", ENERGIES[i]));
    tkhala->Reset();
    tkhala->Add(Form("idaten_hpge_magnet_extended_%dkeV.root", ENERGIES[i]));
    ttigress->Reset();
    ttigress->Add(Form("idaten_hpge_magnet_extended_%dkeV.root", ENERGIES[i]));
  ...
  leg152->Draw();
  c152eu->SaveAs("idaten_hpge_magnet_extended_152Eu_spectrum.png");
  ...
  cg->SaveAs("idaten_hpge_magnet_extended_152Eu_singles_efficiency_2000keV.png");
  
  //  gidaten->GetXaxis()->SetLimits(0, 4500);
  cg->cd()->SetLogx(1);
  gidaten->GetYaxis()->SetNdivisions(11, 5, 0);
  gidaten->GetYaxis()->SetRangeUser(0, 0.21);
  cg->SaveAs("idaten_hpge_magnet_extended_152Eu_singles_efficiency_logx.png");
