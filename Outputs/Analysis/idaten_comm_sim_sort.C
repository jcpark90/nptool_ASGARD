...
void GetStartStopHist(TTree* tr, TH1D* hout, double estart, double ewind_start, double estop, double ewind_stop){
  ...
  long long i = 0;
  //  TH1D* hbgd = (TH1D*)hout->Clone("hbgd"); 
  while(fReader.Next()){           
    i++;
    if((i % 100000)==0) cout << i << endl;
    for(int n = 0; n < Khala->GetKhalaLaBr3EMult();n++){
	...
	// else if (fabs(Khala->GetKhalaLaBr3EEnergy(m)-estart) < 1.5*ewind_start // start gamma in index m
	//     && fabs(Khala->GetKhalaLaBr3EEnergy(n)-estop) < ewind_stop) // stop gamma in index n
	//   hbgd->Fill(Khala->GetKhalaLaBr3TTime(n)-Khala->GetKhalaLaBr3TTime(m));
	// else if (fabs(Khala->GetKhalaLaBr3EEnergy(n)-estart) < 1.5*ewind_start   // start gamma in index n
	// 	 && fabs(Khala->GetKhalaLaBr3EEnergy(m)-estop) < ewind_stop) // stop gamma in index m
	//   hbgd->Fill(Khala->GetKhalaLaBr3TTime(m)-Khala->GetKhalaLaBr3TTime(n));
	// else if (fabs(Khala->GetKhalaLaBr3EEnergy(m)-estart) < ewind_start // start gamma in index m
	//     && fabs(Khala->GetKhalaLaBr3EEnergy(n)-estop) < 1.5*ewind_stop) // stop gamma in index n
	//   hbgd->Fill(Khala->GetKhalaLaBr3TTime(n)-Khala->GetKhalaLaBr3TTime(m));
	// else if (fabs(Khala->GetKhalaLaBr3EEnergy(n)-estart) < ewind_start   // start gamma in index n
	// 	 && fabs(Khala->GetKhalaLaBr3EEnergy(m)-estop) < 1.5*ewind_stop) // stop gamma in index m
	//   hbgd->Fill(Khala->GetKhalaLaBr3TTime(m)-Khala->GetKhalaLaBr3TTime(n));
  ...
  hout->GetXaxis()->SetTitle("Time (ns)");
  return hout;
}
...
void GetLaBr3TimeSpectrum(TTree* tr, TH1D* hlabr3, double epeak, double ewind){
  //  TH1D* hkhala = (TH1D*)hlabr3->Clone("hkhala");
  tr->Project(hlabr3->GetName(), "Khala.fKHALA_LaBr3_T_Time", Form("Khala.fKHALA_LaBr3_E_Energy > %.0lf && Khala.fKHALA_LaBr3_E_Energy < %.0lf", epeak-ewind, epeak+ewind));
  //  hlabr3->Add(hkhala);
  hlabr3->GetXaxis()->CenterTitle();
  hlabr3->GetXaxis()->SetTitle("Time (ns)");
}
