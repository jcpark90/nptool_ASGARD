//11, 15, 18 cm on ASGARD frame, 16 clovers
// CD2 target, thickness = 100 um
// beam energy 10 MeV/u, 15 MeV/u
TChain* tc;
const double EBEAM = 40.*3.7; // 3.7 MeV/u
const double EX = 1.461+0.548; // 1.461-MeV excitation energy for 40Ar
...
//const double TARGET_THICKNESS = 0.001e-4*DENSITY_AU; // physical thickness 2.36 um, should be in in mg/cm2
//const char* FILENAME = "40Ar_197Au_coulex_pencil_beam.root";
const double TARGET_THICKNESS = 2.36e-4*DENSITY_AU; // physical thickness 2.36 um = 2.36e-4 cm, should be in in mg/cm2
const char* FILENAME = "asgard_starkjr_40Ar_pencil.root";
//const double TARGET_THICKNESS = 2.0e-7*DENSITY_AU; // physical thickness 2.36 um = 2.36e-4 cm, should be in in mg/cm2
//const char* FILENAME = "asgard_starkjr_40Ar_thinTarget_pencilBeam.root";
const int ZBEAM = 18;
const int ABEAM = 40;
const int ZTARG = 79;
const int ATARG = 197;
...
void DefineHistos(){
  ...
  ep_z = new TH2D("ep_z", "", 200, -100, 100, EBEAM, 0, EBEAM);
  ep_z->GetXaxis()->SetTitle("z from STARK Jr. (mm)");
  ep_z->GetYaxis()->SetTitle("Particle energy (MeV)");
}
...
void asgard_starkjr_analysis(){
  ...
  while(fReader.Next()){           
    ...
    for(int i = 0; i < STARK->GetMult(); i++){
      double ep_test = STARK->GetFrE(i); // same as GetBkE(i)
      //cout<<i<<" "<<ep_test<<endl;
      //      cout<<i<<" "<<ep_test<<endl;
      if (ep_test > 5.){ // minimum 5 MeV
	...
	TVector3 vSTARK_real = vXYZ_X6[strip].at(det);
  	int pid = BeamOrTarget(EBEAM, ep_test, vSTARK_true.Z());
  	ep[pid] = ep_test;
	if (EXACT_STARKJr_ANGLES){
	  php[pid] = vSTARK_true.Phi();
	  thp[pid] = vSTARK_true.Theta();
	}
	else{
	  vSTARK_real.SetZ(vSTARK_true.Z());
	  php[pid] = vSTARK_real.Phi();
	  thp[pid] = vSTARK_real.Theta();
	}
	if (pid==0){
	  InitializeKinematics(MASS[0], MASS[1], MASS[0], MASS[1], EBEAM, EX);
	  ...
	  //	  cout<<"theta: "<<thp[pid]*180/TMath::Pi()<<" deg, ep: "<<ep[pid]<<" MeV, range: "<<range<<"/"<<TARGET_THICKNESS<<", depth: "<<int_depth<<" mg/cm2"<<endl;
	  ep[pid]+=GetELoss(pid, ep[pid], range, 1);
	  ep_z->Fill(vSTARK_true.Z(), ep[pid]);
	  ...
	  if (int_depth < 0)
	    int_depth = 0.;
	  //	  cout<<"theta: "<<thp[pid]*180/TMath::Pi()<<" deg, ep: "<<ep[pid]<<" MeV, range: "<<range<<", depth: "<<int_depth<<"/"<<TARGET_THICKNESS<<endl;
	  ep[pid]+=GetELoss(pid, ep[pid], range, 1);
	  ep_z->Fill(vSTARK_true.Z(), ep[pid]);
	  beta[pid] = GetBeta(ep[pid], MASS[pid]);
	}
    ...
    if (ep[0] < 0 && ep[1] > 0){ // only 197Au detected
      ...
      if (php[1] > 0)
	php[0] = php[1]-TMath::Pi(); // assume symmetry;
      else
	php[0] = php[1]+TMath::Pi(); // assume symmetry;
    ...
    else if (ep[1] < 0 && ep[0] > 0 ){
      InitializeKinematics(MASS[0], MASS[1], MASS[0], MASS[1], ebeam_attenuated, EX); // change to target as ejectile
    ...
    if (np > 0){
  	...
  	hraw_asgard->Fill(eg_raw); // get the fastest time hit on the plastic detector
	...
	//   cout<<php[0]<<" "<<php[1]<<endl;
	  ...
	  // if (GetDopplerGamma(eg_raw, beta[1], thp[1], php[1], thg, phg) > 534 && GetDopplerGamma(eg_raw, beta[1], thp[1], php[1], thg, phg) < 548)
  ...
  //hbeta->Draw();
  TCanvas* ckin = new TCanvas("ckin", "ckin", 1200, 800);
  ep_z->Draw("colz");
  ...
  latex->DrawLatex(69, 150, "^{197}Au X-ray");
  latex->DrawLatex(275, 140, "^{197}Au 269/279 keV");
  latex->DrawLatex(548+35, 180, "^{197}Au 548 keV");
  latex->DrawLatex(1461, 90, "^{40}Ar 1461 keV");
  ...
  legend->Draw();
  TF1* f40Ar = new TF1("f40Ar", "gaus(0)+pol1(3)", 1461-80, 1461+80);
  f40Ar->SetParameter(1, 1461);
  f40Ar->SetParameter(2, 10);
  hdop_asgard_40Ar->Fit(f40Ar, "QRN0");
  f40Ar->SetLineColor(1);
  f40Ar->Draw("same");
  latex->SetTextAngle(0);
  latex->DrawLatex(700, 90, Form("FWHM(1461) = %.2f keV", fabs(f40Ar->GetParameter(2))*2.355));
